...
try:
    # Try to import inf from torch._six for older versions of PyTorch
    from torch._six import inf
except ImportError:
    # If that fails, import inf directly from torch in newer versions
...
class TensorboardLogger(object):
    ...
    def log_hparams_with_metrics(self, hparams, metrics):
        # Log both hyperparameters and final metrics
        self.writer.add_hparams(hparam_dict=hparams, metric_dict=metrics)

    def _log_start_time(self):
        # Log the start time
        start_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        self.writer.add_text('Experiment/Start Time', f'Experiment started at: {start_time}')
...
def load_state_dict(model, state_dict, prefix='', ignore_missing="relative_position_index"):
    ...
    if len(missing_keys) > 0:
        ...
        if len(missing_keys) > 10:
            print("[DEBUG/INFO] Unusually many missing keys: Maybe model_key needs adjustment. ",
...
def load_model_and_may_interpolate(ckpt_path, model, model_key, model_prefix, is_eval=False):
    print("[DEBUG/INFO] load_model_and_may_interpolate")
...
def merge_batch_tensors_by_dict_key(batch):
    ...
    for tensor_key in batch[0]:
        ...
        elif 'label' in tensor_key or 'slide' in tensor_key :
            batch_tensors[tensor_key] = torch.concat([torch.stack(d[tensor_key]) for d in batch])
        elif  isinstance(batch[0][tensor_key][0], torch.Tensor):
            batch_tensors[tensor_key] = torch.concat([torch.stack(d[tensor_key]) for d in batch])
        else:
            batch_tensors[tensor_key] = torch.stack(torch.tensor([d[tensor_key] for d in batch], dtype=torch.float))
...
def compute_global_target_stats(datasets):
    """
    Compute global mean and standard deviation across multiple datasets.

    Args:
        datasets (list): List of dataset instances with a compute_target_stats method.

    Returns:
        global_mean (float): Global mean of target values.
        global_std (float): Global standard deviation of target values.
    """
    total_mean = 0.0
    total_var = 0.0
    total_count = 0
    ...
    for dataset in datasets:
        mean, std = dataset.compute_target_stats()
        var = std ** 2
        count = len(dataset)

        # Update total mean and variance using the online update formula
        new_total_count = total_count + count
        if total_count > 0:  # Avoid division by zero for the first dataset
            delta = mean - total_mean
            total_mean += delta * count / new_total_count
            total_var = ((total_count * total_var + count * var) + 
                         (delta ** 2) * total_count * count / new_total_count) / new_total_count
        else:
            total_mean = mean
            total_var = var
        total_count = new_total_count

    global_std = total_var ** 0.5
    return total_mean, global_std
